...
> This script is written for automatically preparing the .yaml and .sh files for the datasets present in the dataset-camel.
> As of date this program supports only .csv files
> This program only runs in commandPrompt/terminal and not on IDE's like spyder/pycharm etc..,
> This script prompts the user to select the domain that the user wish to run along with 'Run All' option.
> If user selects any specific domain then the program prompts to run all the datasets present in that domain or the selected datasets.
> If the user selects to choose the datasets he has to run, enter the datasets seperated by commas.
> Finally, the script generates the .yaml and .sh files for each dataset.
> Reference Links used for this script are documented at the end of this file.
> Git Link for this file: https://github.com/RandomTreesAI/cs-skills/tree/master/dataset-camel

To Do:
> Support for .json files
> Description for each user defined function
> Handling scenario if there are multiple files in a dataset.
"""
...
def tag_service_provider(provider):
    ...
    elif "crowd" in last_element: return ".crowdflower","Crowdflower"
...
def goto(domain_dir,run_selected_datasets):
    ...
    inventory_df = inventory_df[inventory_df['datasource_name'].str.contains(domain_dir.split("\\")[-1],case = False)]
    ...
    try:
        counter = 1
        for i in run_selected_datasets:
            process_area_tag = inventory_df[inventory_df['dataset_name_in_yaml'] == "cortex/" + i.replace("-", "_")]['Process Area'].values
            dataset_title = inventory_df[inventory_df['dataset_name_in_yaml'] == "cortex/" + i.replace("-", "_")]['title_scrapped'].values
            dataset_desc = inventory_df[inventory_df['dataset_name_in_yaml'] == "cortex/" + i.replace("-", "_")]['description_scrapped'].values
            dataset_industry = inventory_df[inventory_df['dataset_name_in_yaml'] == "cortex/" + i.replace("-", "_")]['Industry'].values
                ...
                if len(files) > 0:
                    try:
                        for j in files:
                            ...
                            if j.endswith(".csv"):
                                try:
                                    csv_yaml_file = pd.read_csv(subdir + "\\" + j,low_memory=False,error_bad_lines=False,warn_bad_lines=False,encoding = 'ISO-8859-1',nrows=10)
                                    ...
                                    if (len(datatype_list) != len(colname_list)):
                                        ...
                                        yaml_file.write("name: cortex/" + i.replace("-", "_") + "\n")
                                        ...
                                        yaml_file.write("  -\n")
                                        yaml_file.write("    label: dataset.title."+i.replace("-", "_")+"\n")
                                        yaml_file.write("    value: "+i.replace("-", " ").title()+"\n")
...
def main():
   title_1 = 'Kindly choose the Domain you wish to Run: '
   script_dir = os.getcwd()
   domain_list = os.listdir(script_dir +"\\datasets")
   domain_list = [i for i in domain_list if "datagov" in i]
   domain_list_show = domain_list + ['Run All']
   domain_option,domain_index = pick(domain_list_show, title_1)

   if (domain_option != "Run All"):
       domain_dir = script_dir+"\\datasets\\"+domain_option
       print "\nExecuting:",domain_option,"\n"
       title_2 = "Do you wish to run all the datasets or selected ones..??"
       dataset_option_list = ['Run All','Run Selected']
       dataset_option, dataset_index = pick(dataset_option_list, title_2)
       ...
       print "\nOk, Running for all the Domains.\n"
       for each_domain in domain_list:
           domain_dir = script_dir+"\\datasets\\"+each_domain
           print "\tExecuting:",each_domain
           run_selected_datasets = os.listdir(domain_dir)
           goto(domain_dir,run_selected_datasets)
       print "\nExecution Completed..."
...
if __name__ == "__main__":
    ...
    f_0 = open("errorLog_deploy_datagov.txt", "w")
    f_1 = open("deploy_datagov_check.txt", "w")
    main()
...
http://pandas.pydata.org/pandas-docs/stable/generated/pandas.read_json.html; http://pandas.pydata.org/pandas-docs/stable/io.html#io-jsonl
